 ...
 * \tparam wmo_multi_core Generate extra memory barrier instructions in case of
 * weak core to core communication \tparam cacheline_size Size of the cache
 * line, to insert appropriate padding in between indexes and buffer \tparam
 * index_t Type of array indexing type. Serves also as placeholder for future
 * implementations.
 */
template <typename T, size_t buffer_size = 16, bool wmo_multi_core = false,
...
class Ringbuffer {
 ...
 public:
   ...
   * instantiated on stack, heap or inside noinit section then the buffer have
  ...
  void producerClear(void) const {
    if (wmo_multi_core) std::atomic_thread_fence(std::memory_order_acquire);
    head = tail;
  }
   ...
   * \return Indicates if callback was called and data was inserted into
   * internal buffer
   */
  bool insert(T data) {
    ...
    if ((tmp_head - tail) == buffer_size)
      ...
      if (wmo_multi_core) std::atomic_thread_fence(std::memory_order_release);
   ...
   * into internal buffer, is located \return Indicates if callback was called
   * and data was inserted into internal buffer
   */
  ...
  T* get_next_write_slot() {
    ...
    if ((head - tail) == buffer_size) return nullptr;
   ...
   * clearing status flags by reading a data register)
   *
   * \param acquire_data_callback Pointer to callback function that returns
   * element to be inserted into buffer \return Indicates if callback was called
   * and data was inserted into internal buffer
   */
   ...
   * stored \return Indicates if data was fetched from the internal buffer
   */
  bool remove(T& data) {
   ...
   * elements written into circular buffer
   */
 ...
 private:
  ...
  static_assert(!(buffer_size & buffer_mask),
...
template <typename T, size_t buffer_size, bool wmo_multi_core,
          size_t cacheline_size, typename index_t>
size_t Ringbuffer<T, buffer_size, wmo_multi_core, cacheline_size,
                  index_t>::writeBuff(const T* buff, size_t count) {
  index_t available = 0;
  index_t tmp_head = head;
...
size_t Ringbuffer<T, buffer_size, wmo_multi_core, cacheline_size,
  ...
  while (written < count) {
    ...
    if (execute_data_callback) execute_data_callback();
...
template <typename T, size_t buffer_size, bool wmo_multi_core,
          size_t cacheline_size, typename index_t>
size_t Ringbuffer<T, buffer_size, wmo_multi_core, cacheline_size,
                  index_t>::readBuff(T* buff, size_t count) {
  index_t available = 0;
