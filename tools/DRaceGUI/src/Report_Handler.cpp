...
/// checks if the report converter is in the current directory or in the
/// ../ReportConverter/ dir
...
/// evaluates if name is a valid ReportConverter (must be called with existing
/// files)
...
/// returns true when name, report converter is valid and the checkbox "report"
/// is set
...
void Report_Handler::set_create_state(bool state) { create_report = state; }
