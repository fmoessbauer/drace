...
class Statistics {
 ...
 public:
  ...
  explicit Statistics(thread_id_t tid)
      : page_hits(0.01, 0.001), pc_hits(0.01, 0.001) {}
  ...
  void print_summary(file_t target) {
    ...
    if (flushes > 0) {
      s << "avg. buffer size:\t" << std::dec << (total_refs / flushes)
        << std::endl;
    }
    s << "e-flushes:\t\t" << std::dec << external_flushes << std::endl
      ...
      << "mod. load time(total):\t" << std::dec << module_load_duration.count()
      << "ms" << std::endl;
    s << "top pages:\t\t";
    ...
    for (const auto& p : freq_pc_hist) {
      double share =
          (static_cast<double>(p.second) / state.N) * (100.0 + state.e);
      s << "(" << std::setw(8) << std::hex << p.first << "," << std::dec
        << std::setprecision(3) << std::setw(2) << share << "%),";
    }
