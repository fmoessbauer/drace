...
std::string Symbols::get_bb_symbol(app_pc pc) {
  ...
  if (modptr) {
    ...
    for (; offset >= limit; --offset) {
      drsym_error_t err = drsym_lookup_address(modptr->info->full_path, offset,
                                               &syminfo, DRSYM_DEMANGLE);
...
#endif
  if (modptr &&
      ((modptr->modtype == module::Metadata::MOD_TYPE_FLAGS::NATIVE) ||
       (modptr->modtype == module::Metadata::MOD_TYPE_FLAGS::MANAGED &&
        !shmdriver))) {
