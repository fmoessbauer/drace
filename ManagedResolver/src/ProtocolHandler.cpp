...
BOOL SymbolMatchCallback(PSYMBOL_INFO pSymInfo, ULONG SymbolSize,
                         PVOID UserContext) {
...
ProtocolHandler::ProtocolHandler(SyncSHMDriver shmdriver, bool once)
    : _shmdriver(shmdriver), _exec_once(once) {
  ...
  symloadmod =
      (PFN_SymLoadModuleExW)GetProcAddress(_dbghelp_dll, "SymLoadModuleExW");
  symunloadmod =
      (PFN_SymUnloadModule)GetProcAddress(_dbghelp_dll, "SymUnloadModule");
  symgetmoduleinfo = (PFN_SymGetModuleInfoW64)GetProcAddress(
      _dbghelp_dll, "SymGetModuleInfoW64");
  ...
  symgetsearchpath =
      (PFN_SymGetSearchPath)GetProcAddress(_dbghelp_dll, "SymGetSearchPath");

  _shmdriver->id(SMDataID::READY);
  _shmdriver->commit();
}

ProtocolHandler::~ProtocolHandler() { quit(); }
...
void ProtocolHandler::attachProcess() {
  ...
  auto result = std::async([&]() {
    return _resolver.InitSymbolResolver(_phandle, dacdllpath.c_str(),
                                        lastError);
  });
...
void ProtocolHandler::getCurrentStack() {
  ...
  // TODO: It is unclear if the stack-walk also works if the thread is not
  // suspended
  //
  ...
  if (GetThreadContext(hThread, &context)) {
    ...
    HRESULT hr = debugCtrl->GetContextStackTrace(
        &context, sizeof(CONTEXT), frames, 32, contexts, 32 * sizeof(CONTEXT),
        sizeof(CONTEXT), &filled);
    if (hr == S_OK) {
      for (unsigned i = 0; i < filled; ++i) {
        ...
        logger->info("+ {}: Rip is {}, symbol: {} @ {}", i,
                     (void*)contexts[i].Rip, symbol.GetBuffer(128),
                     line.GetBuffer(128));
      }
    } else {
...
void ProtocolHandler::resolveIP() {
  ...
  bs = sym.module.size();
  strncpy_s(sym.module.data(), bs, buffer.GetBuffer(static_cast<int>(bs)),
            _TRUNCATE);
  ...
  strncpy_s(sym.function.data(), bs, buffer.GetBuffer(static_cast<int>(bs)),
            _TRUNCATE);
  ...
  bs = sym.path.size();
  strncpy_s(sym.path.data(), bs, buffer.GetBuffer(static_cast<int>(bs)),
            _TRUNCATE);
...
void ProtocolHandler::init_symbols() {
  ...
  DWORD symopts =
      symgetopts() | SYMOPT_LOAD_LINES | SYMOPT_UNDNAME | SYMOPT_EXACT_SYMBOLS;
...
void ProtocolHandler::loadSymbols() {
  ...
  auto symload = std::async([&]() {
    return symloadmod(_phandle, NULL, wstrpath.c_str(), NULL, sr.base,
                      (DWORD)sr.size, NULL, 0);
  });
    ...
    if (symgetmoduleinfo(_phandle, sr.base, &info)) {
      switch (info.SymType) {
        case SymNone:
          ...
          break;
        case SymExport:
          logger->debug("Only export symbols found");
          break;
        case SymPdb:
          logger->debug("Loaded pdb symbols");
          break;
        case SymDeferred:
          logger->debug("Symbol load deferred");
          break;
        case SymCoff:
        case SymCv:
        case SymSym:
        case SymVirtual:
        case SymDia:
          logger->debug("Symbols in image file loaded");
          break;
        default:
          logger->debug("Symbols in unknown format");
          break;
      }
...
void ProtocolHandler::searchSymbols() {
  ...
  logger->debug("search symbols matching {} at {} - full: {}",
                symreq.match.data(), (void*)symreq.base,
                symreq.full ? "no" : "yes");
  if (symsearch(_phandle, symreq.base, 0, 0, symreq.match.data(), 0,
                SymbolMatchCallback, (void*)&symbol_addrs,
                symreq.full ? SYMSEARCH_ALLITEMS : SYMSEARCH_GLOBALSONLY)) {
    auto& symresp =
        _shmdriver->emplace<ipc::SymbolResponse>(ipc::SMDataID::SEARCHSYMS);
    ...
    auto cpy_range_end = symresp.size <= symresp.adresses.size()
                             ? symbol_addrs.end()
                             : (symbol_addrs.begin() + symresp.adresses.size());
...
void ProtocolHandler::quit() { _keep_running = false; }
