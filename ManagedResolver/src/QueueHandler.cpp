...
void QueueHandler::process(int queueid) {
  ...
  SetThreadAffinityMask(GetCurrentThread(),
                        1 << (queueid % std::thread::hardware_concurrency()));
...
void QueueHandler::_acquire(ipc::event::Mutex* entry, int queueid) {
  ...
  if (entry->write) {
    __tsan_MutexLock(thr, 0, (void*)lower_half(entry->addr), entry->recursive,
                     false);
  } else {
...
void QueueHandler::_read(ipc::event::MemAccess* entry, int queueid) {
  logger->trace("read: tid {}, addr {}, stacksize {}", entry->thread_id,
                (void*)entry->addr, entry->stacksize);

  void* thr = _tids[queueid][entry->thread_id];
  if (nullptr == thr) return;
  __tsan_read(thr, (void*)lower_half(entry->addr), kSizeLog1,
              entry->callstack.data(), entry->stacksize);
}
void QueueHandler::_write(ipc::event::MemAccess* entry, int queueid) {
  logger->trace("write: tid {}, addr {}, stacksize {}", entry->thread_id,
                (void*)entry->addr, entry->stacksize);

  void* thr = _tids[queueid][entry->thread_id];
  if (nullptr == thr) return;
  __tsan_write(thr, (void*)lower_half(entry->addr), kSizeLog1,
               entry->callstack.data(), entry->stacksize);
}
void QueueHandler::_allocate(ipc::event::Allocation* entry, int queueid) {
  logger->trace("allocate: tid {}, addr {}, size {}", entry->thread_id,
                entry->addr, entry->size);
  __tsan_malloc_use_user_tid(entry->thread_id, 0x42, lower_half(entry->addr),
                             entry->size);
}
