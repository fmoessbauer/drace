 ...
 *
 * Based on
 * https://stackoverflow.com/questions/34733155/resolve-managed-and-native-stack-trace-which-api-to-use
 */
...
ManagedResolver::~ManagedResolver() { Close(); }

void ManagedResolver::Close() {
  if (debugClient) debugClient->DetachProcesses();
...
bool ManagedResolver::InitSymbolResolver(HANDLE hProcess,
  ...
  if (_mscordacwks_dll != 0)
    pCLRCreateInstance = (PFN_CLRDataCreateInstance)GetProcAddress(
        _mscordacwks_dll, "CLRDataCreateInstance");

  if (_mscordacwks_dll == 0 || pCLRCreateInstance == 0) {
    lastError.Format(
        _T("Required dll mscordaccore.dll from .core2.0 installation was not ")
        _T("found"));
    Close();
    return false;
  }
  ...
  HRESULT hr = pCLRCreateInstance(__uuidof(IXCLRDataProcess), target,
                                  (void**)&clrDataProcess);

  if (hr != S_OK) {
    lastError.Format(
        _T("failed to initialize Dotnet debugging instance: Err (Error code: ")
        _T("0x%08X)"),
  ...
  PFN_DebugCreate* pDebugCreate =
      (PFN_DebugCreate*)GetProcAddress(_dbgeng_dll, "DebugCreate");

  hr = pDebugCreate(__uuidof(IDebugClient), (void**)&debugClient);
  if (FAILED(hr)) {
    lastError.Format(
        _T("Could retrieve symbolic debug information using dbgeng.dll (Error ")
        _T("code: 0x%08X)"),
        hr);
    Close();
    return false;
  ...
  if (hr != S_OK) {
    lastError.Format(_T("Could attach to process 0x%X (Error code: 0x%08X)"),
                     processId, hr);
    Close();
    return false;
  }
...
bool ManagedResolver::GetModuleFileLineInfo(void* ip, CStringA* lineInfo,
                                            uint64_t* lineNumber,
                                            CStringA* modulePath) {
  USES_CONVERSION;
  ULONG lineN = 0;
  ...
  if (!debugSymbols || !debugSymbols3) return false;

  // Get managed method by address
  CLRDATA_ENUM methEnum;
  // TODO: This does not work for coreclr (reason is unclear even after massive
  // debugging)
  HRESULT hr = clrDataProcess->StartEnumMethodInstancesByAddress(
      (CLRDATA_ADDRESS)ip, NULL, &methEnum);
  if (hr == S_OK) {
    hr = clrDataProcess->EnumMethodInstanceByAddress(&methEnum, &method);
    clrDataProcess->EndEnumMethodInstancesByAddress(methEnum);
  }

  if (!method) goto lDefaultFallback;
  ...
  switch ((long)ilOffsets) {
    ...
    case CLRDATA_IL_OFFSET_EPILOG: {
      ...
      for (;;) {
        ...
        if (needed <= count || map != mapLocal) break;
      ...
      for (unsigned i = 0; i < needed; i++) {
        ...
        if (l == CLRDATA_IL_OFFSET_NO_MAPPING ||
            l == CLRDATA_IL_OFFSET_PROLOG || l == CLRDATA_IL_OFFSET_EPILOG)
          continue;

        if (map[i].ilOffset > highestOffset) highestOffset = map[i].ilOffset;
      }  // for

      if (map != mapLocal) delete[] map;
  ...
  void* moduleBase = 0;
  {
    ...
    if (!module) goto lDefaultFallback;
    ...
    //
    // Retrieve ImageInfo associated with the IXCLRDataModule instance passed
    // in. First look for NGENed module, second for IL modules.
    //
    for (int extentType = CLRDATA_MODULE_PREJIT_FILE;
         extentType >= CLRDATA_MODULE_PE_FILE; extentType--) {
      CLRDATA_ENUM enumExtents;
      if (module->StartEnumExtents(&enumExtents) != S_OK) continue;

      CLRDATA_MODULE_EXTENT extent;
      while (module->EnumExtent(&enumExtents, &extent) == S_OK) {
        logger->debug("+ EnumExtent @ {}, type {}", (void*)extent.base,
                      extent.type);
        if (extentType != extent.type) continue;

        ULONG startIndex = 0;
        ULONG64 modBase = 0;

        hr = debugSymbols3->GetModuleByOffset((ULONG64)extent.base, 0,
                                              &startIndex, &modBase);
        logger->debug("++ Extent: base {}, length {}, status {}",
                      (void*)extent.base, extent.length, hr);
  ...
  hr = debugSymbols3->GetSymbolEntryByToken((ULONG64)moduleBase, methodToken,
                                            &id);
  if (FAILED(hr)) goto lDefaultFallback;
  ...
  for (ULONG SkipCount = 64; SkipCount > 0; SkipCount--) {
    hr = debugSymbols3->GetLineByOffset((ULONG64)IlOffset, &lineN, path,
                                        sizeof(path), NULL, &dispacement);
    if (FAILED(hr)) break;
...
lDefaultFallback:
  if (lineInfo == NULL) return true;

  hr = debugSymbols3->GetLineByOffset((ULONG64)ip, &lineN, path, sizeof(path),
                                      NULL, &dispacement);
...
bool ManagedResolver::GetMethodName(void* ip, CStringA& symbol) {
  ...
  ULONG32 len = 0;
  wchar_t name[1024];
  if (!clrDataProcess) return false;

  HRESULT hr = clrDataProcess->GetRuntimeNameByAddress(
      (CLRDATA_ADDRESS)ip, 0, sizeof(name) / sizeof(name[0]), &len, name,
      &displacement);
...
void ManagedResolver::getStackTrace(void* ip) {
  ...
  ULONG frames_filled;
  HRESULT hr =
      debugControl->GetStackTrace(0, 0, 0, frames, frames_size, &frames_filled);
  if (hr == S_OK) {
