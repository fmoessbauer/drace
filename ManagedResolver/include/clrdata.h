 ...
 public:
  ...
  HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, PVOID *ppvObject) {
    if (IsEqualIID(riid, IID_IUnknown) ||
        IsEqualIID(riid, __uuidof(ICLRDataTarget))) {
      AddRef();
      *ppvObject = this;
      return S_OK;
    }
  ...
  virtual HRESULT STDMETHODCALLTYPE GetImageBase(LPCWSTR imagePath,
    ...
    if (bIsWow64) whatToList = LIST_MODULES_32BIT;

    if (!EnumProcessModulesEx(hProcess, dlls, sizeof(dlls), &nItems,
                              whatToList)) {
    ...
    for (unsigned int i = 0; i < nItems; i++) {
      path[0] = 0;
      if (GetModuleFileNameExW(hProcess, dlls[i], path,
  ...
  virtual HRESULT STDMETHODCALLTYPE ReadVirtual(CLRDATA_ADDRESS address,
                                                BYTE *buffer,
                                                ULONG32 bytesRequested,
                                                ULONG32 *bytesRead) {
    SIZE_T readed;

    if (!ReadProcessMemory(hProcess, (void *)address, buffer, bytesRequested,
                           &readed))
  ...
  virtual HRESULT STDMETHODCALLTYPE WriteVirtual(CLRDATA_ADDRESS address,
                                                 BYTE *buffer,
                                                 ULONG32 bytesRequested,
  ...
  virtual HRESULT STDMETHODCALLTYPE GetTLSValue(ULONG32 threadID, ULONG32 index,
                                                CLRDATA_ADDRESS *value) {
    return E_NOTIMPL;
  }

  virtual HRESULT STDMETHODCALLTYPE SetTLSValue(ULONG32 threadID, ULONG32 index,
                                                CLRDATA_ADDRESS value) {
    return E_NOTIMPL;
  }
  ...
  virtual HRESULT STDMETHODCALLTYPE GetThreadContext(ULONG32 threadID,
  ...
  virtual HRESULT STDMETHODCALLTYPE SetThreadContext(ULONG32 threadID,
                                                     ULONG32 contextSize,
                                                     BYTE *context) {
    return E_NOTIMPL;
  }

  virtual HRESULT STDMETHODCALLTYPE Request(ULONG32 reqCode,
